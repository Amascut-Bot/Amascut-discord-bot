...
export default class MessageUpdate extends BotEvent {
    ...
    async run(oldMessage: Message, newMessage: Message): Promise<any> {
        ...
        if (teamformingChannels.includes(newMessage.channelId) && (newMessage.content.toLowerCase().includes('keep') || newMessage.content.toLowerCase().includes('keeps'))) {
            await newMessage.reply('Use <#1413114658541539410> for keeps!');
