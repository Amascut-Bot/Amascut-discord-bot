import { Attachment, ComponentType, Interaction, Message, MessageFlags, MessageType, TextChannel } from "discord.js";
import Bot from "../Bot";
...
export default class UploadHandler {
    ...
    public async uploadFile(channel: TextChannel, attachment: Attachment, interaction: Interaction) {
        ...
        try {
            ...
            this.client.logger.error({
                message: 'INFO: message-tags.json not found or invalid, starting fresh. This is expected on first run.',
        ...
        try {
            ...
            const archiveFileName = `${timestamp}_${channel.id}.txt`;
        ...
        if (!fileContent.trim()) {
            throw 'The uploaded file is empty.';
        }
        ...
        if ('editReply' in interaction) {
            await interaction.editReply({
                content: `Successfully parsed your file. Sending ${sentMessageCount} message(s) to ${channel === interaction.channel ? 'this channel' : `<#${channel.id}>`}. This may take a moment...`
            });
        }
        ...
        if (logChannelId && logChannelId !== 'YOUR_CHANNEL_ID_HERE') {
            ...
            if (logChannel) {
                await logChannel.send({
                    content: `${interaction.user.username} uploaded to <#${channel.id}>`,
                    files: [attachment]
                });
            }
        }
        ...
        try {
            for (const part of parsedParts) {
                ...
                const finalContent = content ? this.convertEmojis(content) : '';
                const finalEmbeds = embeds.map(embed => this.convertEmbedEmojis(embed));
                const finalComponents = components.map(comps => this.convertComponentsV2Emojis(comps));
                ...
                if (part.nameTag) {
                    ...
                    const oldMessageId = Object.keys(tags[channel.id]).find(
                        (msgId) => tags[channel.id][msgId] === part.nameTag
                    );
                    if (oldMessageId) {
                        ...
                        delete tags[channel.id][oldMessageId];
                    }

                    tags[channel.id][sentMessage.id] = part.nameTag;
        ...
        if (messagesToEdit.length > 0) {
            for (const partToEdit of messagesToEdit) {
                ...
                if (newContent) {
                    newContent = this.resolvePlaceholders(newContent, tagToUrlMap, { channelId: channel.id });
                    newContent = this.convertEmojis(newContent);
                }

                // Resolve placeholders in embeds too
                const newEmbeds = partToEdit.embeds.map(embed => {
                    let embedString = JSON.stringify(embed);
                    embedString = this.resolvePlaceholders(embedString, tagToUrlMap, { channelId: channel.id });
                    const resolvedEmbed = JSON.parse(embedString);
                    return this.convertEmbedEmojis(resolvedEmbed);
                });
                ...
                const newComponents = partToEdit.components.map(component => {
                    ...
                    componentString = this.resolvePlaceholders(componentString, tagToUrlMap, { channelId: channel.id });
                    const resolvedComponent = JSON.parse(componentString);
                    return this.convertComponentsV2Emojis(resolvedComponent);
                });
    ...
    public async editMessage(message: Message, attachment: Attachment) {
        const response = await fetch(attachment.url);
        const fileContent = await response.text();
        ...
        if (parsedParts.length > 1) {
            throw 'You can only edit 1 message at a time!';
        }
        ...
        try {
            for (const part of parsedParts) {
                ...
                if (finalComponents.length > 0) {
                    ...
                    // if message was already a cv2 container then this will hard fail
                    sentMessage = await message.edit({
                        content: finalContent || undefined,
                        embeds: finalEmbeds,
                        allowedMentions: { "parse": [] }
    ...
    public splitMessage(text: string, maxLength = 1900): string[] {
    ...
    public async checkForLinkErrors(parsedParts: ParsedMessage[], interaction: Interaction): Promise<boolean> {
        ...
        if (linkErrors.length > 0) {
            if ('editReply' in interaction && 'followUp' in interaction) {
    ...
    public parseFileContent(content: string): ParsedMessage[] {
    ...
    public resolvePlaceholders(text: string, tagMap: Map<string, string>, dynamicData: { channelId: string }): string {
        if (!text) return text;
    ...
    public convertEmbedEmojis(embed: any): any {
        if (!embed || typeof embed !== 'object') return embed;

        const newEmbed = JSON.parse(JSON.stringify(embed));

        const convert = (text: string) => text ? this.convertEmojis(text) : text;
    ...
    public convertComponentsV2Emojis(container: any): any {
    ...
    public convertEmojis(text: string): string {
        if (!text) return text;
