            ...
            try {
                ...
                switch (command.permissions) {
                    case 'BOT_OWNER':
                        ...
                        break;
                    case 'OWNER':
                        if (!(await this.client.util.hasRolePermissions(this.client, ['owner'], interaction))) {
                            this.client.logger.log({
                                message: `Attempted restricted permissions. { command: ${command.name}, user: ${interaction.user.username}, channel: ${interaction.channel} }`,
                                handler: this.constructor.name,
                            }, true);
                    ...
                    case 'TRIAL_TEAM':
                        ...
                        break;
                    case 'TEACHER_LOREBOOK_TRIAL_TEAM':
                        if (!(await this.client.util.hasRolePermissions(this.client, ['teacher', 'lorebook', 'trialTeam', 'admin', 'owner'], interaction))) {
                            this.client.logger.log({
                                message: `Attempted restricted permissions. { command: ${command.name}, user: ${interaction.user.username}, channel: ${interaction.channel} }`,
                                handler: this.constructor.name,
                            }, true);
